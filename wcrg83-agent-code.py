...
class SACAgent(torch.nn.Module):
    ...
    def train_step(self):
        ...
        # Critic update:
        with torch.no_grad():
            # Sample action from current policy for next state 
...
env = rld.Recorder(
    ...
    video_prefix="wcrg83-agent-video",  # prefix for videos (replace xxxx00 with your username)
    logs=True,                          # keep logs
)
...
env.close()
env.write_log(folder="logs", file="wcrg83-agent-log.txt")
